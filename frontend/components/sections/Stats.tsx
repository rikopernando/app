...
const StatCard = ({
  ...
  return (
    ...
    >
      ...
      <p className={`mt-3 text-[14px] font-semibold ${isFilled ? "text-white/90" : "text-ink-soft"}`}>
        {label}
      </p>
    </div>
  );
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-10">
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8 mb-14">
          <div className="max-w-xl">
            <span className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-sky-50 text-sky-600 text-[11px] tracking-[0.18em] uppercase font-bold">
              <span className="w-1.5 h-1.5 rounded-full bg-sky" /> Statistik Sekolah
            </span>
            <h2
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 lg:gap-6" data-testid="stats-grid">
          {STATS.map((s) => (
