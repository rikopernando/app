...
export const PPDB = () => {
  const handleDownload = () => {
    toast.success("Formulir PPDB sedang diunduh", {
      description:
        "Lengkapi dan kirim kembali ke sekolah. Butuh bantuan? Hubungi (0723) 465335.",
      duration: 5000,
    });
  };
          ...
          >
            ...
            <ol className="relative space-y-6 before:absolute before:left-[26px] before:top-4 before:bottom-4 before:w-px before:bg-white/15">
              {TIMELINE.map((t, i) => {
                ...
                return (
                  <li key={i} data-testid={`ppdb-timeline-${i + 1}`} className="relative flex gap-5 group">
                    <div className="flex-shrink-0 relative z-10">
                      ...
                      >
                        <span className={`font-display text-[14px] font-bold ${tone.text}`}>{t.date}</span>
                        <span className={`text-[9px] tracking-wide font-bold ${tone.sub}`}>{t.month}</span>
                      </div>
                    </div>
                    <div className="flex-1 pt-1 pb-2">
                      ...
                      <p className="mt-2 text-[14px] leading-relaxed text-cream-300 max-w-md">{t.desc}</p>
                    </div>
                  </li>
                );
              })}
          ...
          <div className="lg:col-span-5 space-y-6">
            <div data-testid="ppdb-requirements" className="rounded-[32px] p-8 lg:p-10 bg-white text-ink shadow-soft-lg">
              <div className="flex items-center gap-2.5 mb-6">
                ...
                <div>
                  ...
                  <h3 className="font-display text-xl text-ink leading-tight">Syarat Pendaftaran</h3>
                </div>
              </div>
              ...
              <ul className="space-y-3">
                {REQUIREMENTS.map((r, i) => (
                  ...
                  >
                    <CheckCircle2 size={18} className="text-mint-600 flex-shrink-0 mt-0.5" />
                    {r}
                  </li>
                ))}
              </ul>
            ...
            <div data-testid="ppdb-download-card" className="relative rounded-[32px] p-8 bg-gradient-to-br from-coral to-coral-600 text-white overflow-hidden shadow-glow">
              ...
              <div className="relative">
                ...
                <h3 className="font-display text-2xl leading-[1.15]">Formulir Pendaftaran PPDB</h3>
