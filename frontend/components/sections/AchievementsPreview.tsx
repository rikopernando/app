...
const COLOR_MAP = ["navy", "sky", "gold", "mint", "sky"] as const;
type Color = (typeof COLOR_MAP)[number];

const COLOR: Record<Color, { bg: string; text: string; subText: string; chip: string }> = {
  navy: { bg: "bg-navy", text: "text-white", subText: "text-white/80", chip: "bg-white/20 text-white" },
  sky: { bg: "bg-white", text: "text-ink", subText: "text-ink-soft", chip: "bg-sky-50 text-sky-600" },
  gold: { bg: "bg-white", text: "text-ink", subText: "text-ink-soft", chip: "bg-gold-50 text-gold-600" },
...
export async function AchievementsPreview() {
  ...
  return (
    <section id="prestasi" data-testid="achievements-section" className="relative py-24 lg:py-32 bg-cream-100/40 overflow-hidden">
      <div className="absolute top-20 right-0 w-72 h-72 bg-gold/20 blur-3xl rounded-full" aria-hidden />
          ...
          {preview.map((a, i) => {
            ...
            return (
              ...
              >
                ...
                <p className="mt-8 font-display text-lg leading-none tracking-tight text-navy font-bold">
                  {a.rank}
                </p>
